...
async def cancel_booking(
    ...
    logger.info(
        "Booking cancelled: booking_id=%s, tenant_id=%s",
        booking.id,
        tenant.id
    )
...
async def list_bookings_for_date(
    booking_date: date = Query(default=None, description="Date to fetch (YYYY-MM-DD). Defaults to today."),
    clinic_location_id: Optional[str] = Query(default=None, description="Filter by clinic location"),
    tenant: Tenant = Depends(get_current_tenant),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    return [
        {
            ...
            "clinic_name": b.clinic_location.clinic_name if b.clinic_location else "Unknown",
            "clinic_address": (
...
async def download_schedule_pdf(
    ...
    clinic_location_id: Optional[str] = Query(default=None, description="Clinic location ID"),
