...
class BookingService:
    ...
    async def confirm_booking(
        ...
        logger.info(
            "Confirmed booking %s with payment %s", booking_id, razorpay_payment_id
        )
    ...
    async def cancel_booking(
        ...
        result = await db.execute(
            select(Booking).where(
                and_(Booking.id == booking_id, Booking.tenant_id == tenant_id)
            )
        )
        booking = result.scalar_one_or_none()
    ...
    async def cleanup_expired_reservations(db: AsyncSession) -> int:
        ...
        result = await db.execute(
            select(Booking).where(
                and_(Booking.status == BookingStatus.RESERVED, Booking.expires_at < now)
            )
        )
