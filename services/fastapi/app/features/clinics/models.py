...
class ClinicLocation(Base):
    ...
    )  # e.g., "City Clinic", "Morning Clinic"
    ...
    bookings = relationship(
        "Booking", back_populates="clinic_location", cascade="all, delete-orphan"
    )
    daily_schedules = relationship(
        "DailySchedule", back_populates="clinic_location", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"<ClinicLocation(id={self.id}, name={self.clinic_name}, city={self.city})>"
        )
