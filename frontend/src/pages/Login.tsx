...
import { BellRing, ArrowRight, Info } from 'lucide-react';
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-white px-6">
        <div className="w-full max-w-sm">
          ...
          {!isRegister && (
            <>
              {/* Friendly notice shown when user clicks Google / Facebook */}
              {oauthNotice && (
                <div className="mt-4 flex items-start gap-2 rounded-lg bg-blue-50 border border-blue-100 px-3 py-2.5">
                  <Info className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-blue-700 leading-snug">{oauthNotice}</p>
                </div>
              )}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  onClick={() => setOauthNotice(
                    "Google login isn't configured in this portfolio build — no paid credentials yet! 😊 Please use email + password to sign in."
                  )}
                  className="flex-1 h-10 flex items-center justify-center gap-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={() => setOauthNotice(
                    "Facebook login isn't configured in this portfolio build — no paid credentials yet! 😊 Please use email + password to sign in."
                  )}
                  className="flex-1 h-10 flex items-center justify-center gap-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
                >
